...
const ERROR_MESSAGES: Record<string, string> = {
  missing_code:           "Notion didn't return an authorization code — try again.",
  invalid_state:          "Security check failed. The sign-in link may have expired or been opened in a different browser. Try again.",
  server_misconfigured:   "Arena is missing the Notion app credentials. The administrator needs to set NOTION_CLIENT_ID + NOTION_CLIENT_SECRET on Vercel.",
  ...
  access_denied:          "You declined the Notion permission prompt. Accept it to continue.",
}
      ...
      {existing.length > 0 && (
        <section className="flex flex-col gap-2">
          ...
          {existing.map(c => (
            <Link
              key={c.id}
              href={`/connect/notion/${c.id}/settings`}
            ...
            >
              ...
              {c.workspaceIcon && c.workspaceIcon.startsWith("http") && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={c.workspaceIcon} alt="" className="w-8 h-8 rounded-md flex-shrink-0" />
              )}
              <div className="flex-1 min-w-0">
                <p className="text-base text-[color:var(--color-fg)] truncate">
                  {c.workspaceName || c.label || "Notion connection"}
                </p>
      ...
      <section className="surface p-6">
        {oauthAvailable ? (
          <div className="flex flex-col items-start gap-4">
            <div>
              <p className="text-base font-semibold text-[color:var(--color-fg)]">
                {existing.length === 0 ? "Sign in with Notion" : "Add another workspace"}
              </p>
              <p className="text-sm text-[color:var(--color-fg-muted)] mt-1.5 max-w-md">
                Notion will ask which pages and databases to grant Eve access to. You stay in control.
              </p>
            </div>
            <a href="/api/oauth/notion/start" className="btn btn-primary">
              <Plus size={15} />
              Continue with Notion
              <ArrowRight size={14} className="opacity-70" />
            </a>
            <p className="text-xs text-[color:var(--color-fg-subtle)]">
              Prefer to paste an integration secret manually? <Link href="/connect/notion/manual" className="underline hover:text-[color:var(--color-fg-muted)]">Use the legacy form</Link>.
            </p>
          </div>
        ) : (
          <div className="flex flex-col items-start gap-4 w-full">
            <div>
              <p className="text-base font-semibold text-[color:var(--color-fg)]">
                Notion sign-in isn&apos;t configured yet
              </p>
              <p className="text-sm text-[color:var(--color-fg-muted)] mt-1.5 max-w-md">
                The administrator needs to register a Notion integration once. After that everyone can sign in with one click below.
              </p>
            </div>

            <div className="w-full surface-flush p-4 mt-2">
              <p className="text-sm font-semibold text-[color:var(--color-fg)] mb-3">
                Admin: register the Notion integration
              </p>
              <ol className="flex flex-col gap-2.5 text-sm text-[color:var(--color-fg-muted)] leading-relaxed">
                <li className="flex gap-2.5">
                  ...
                  <span>
                    Open <a href="https://www.notion.so/my-integrations" target="_blank" rel="noopener noreferrer" className="underline">notion.so/my-integrations</a> → click <strong>+ New integration</strong>.
                  </span>
                </li>
                <li className="flex gap-2.5">
                  ...
                  <span>
                    Choose <strong>Public integration</strong> (lets users sign in via OAuth, not just you).
                    Fill in name, logo, redirect URI:
                    <code className="block mt-1.5 text-xs bg-[color:var(--color-bg)] px-2 py-1.5 rounded break-all font-mono">
                      https://arena.maxnexus.io/api/oauth/notion/callback
                    </code>
                  </span>
                </li>
                <li className="flex gap-2.5">
                  ...
                  <span>
                    Save → Notion shows a <strong>OAuth client ID</strong> and <strong>OAuth client secret</strong>. Copy both.
                  </span>
                </li>
                <li className="flex gap-2.5">
                  ...
                  <span>
                    ...
                    <code className="block mt-1.5 text-xs bg-[color:var(--color-bg)] px-2 py-1.5 rounded font-mono">NOTION_CLIENT_ID</code>
                    <code className="block mt-1 text-xs bg-[color:var(--color-bg)] px-2 py-1.5 rounded font-mono">NOTION_CLIENT_SECRET</code>
                  </span>
                </li>
                <li className="flex gap-2.5">
                  ...
                  <span>
                    Vercel will redeploy. Refresh this page — the &ldquo;Continue with Notion&rdquo; button replaces these instructions.
                  </span>
                </li>
              </ol>
              <p className="text-xs text-[color:var(--color-fg-subtle)] mt-3">
                Reference: <a href="https://developers.notion.com/docs/authorization" target="_blank" rel="noopener noreferrer" className="underline">Notion Authorization docs</a>
              </p>
            </div>

            <p className="text-xs text-[color:var(--color-fg-subtle)]">
              In a hurry?{" "}
              <Link href="/connect/notion/manual" className="underline hover:text-[color:var(--color-fg-muted)]">
                Use an Internal Integration secret
              </Link>
              {" "}— works only for your own Notion, doesn&apos;t require a public integration.
            </p>
          </div>
        )}
      </section>
